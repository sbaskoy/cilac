...
class Alanlar {
  ...
  Alanlar({this.id, this.alanAdi, this.kayitSayisi});

  Alanlar.fromJson(Map<String, dynamic> json) {
    id = json['id'];
  ...
  Map<String, dynamic> toJson() {
    ...
    data['id'] = this.id;
    data['alanAdi'] = this.alanAdi;
    data['kayitSayisi'] = this.kayitSayisi;
    return data;
  }

  Alanlar getObj(json) => Alanlar.fromJson(json);

  static Future<List> fromAPI(String token, String data) async {
    final Response response = await post(Uri.parse(API_URL + "api/lst/alanlarlst"),
    ...
    if (response.statusCode == 200) {
      return response.body.toList(new Alanlar());
  ...
  static Future<List> fromLocal() async {
    return await (new AlanlarDb().getAll());
  }
}
