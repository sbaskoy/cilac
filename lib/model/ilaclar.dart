import 'dart:convert';

import '../localdb/ilaclar_db.dart';
import 'package:http/http.dart';

import '../error/errormodel.dart';
import '../global/constant.dart';
import '../global/global_function.dart';

class Ilaclar {
  int id;
  String ilacAdi;
  String birimi;
  String ikinciBirimi;
  double katsayi;
  int kayitSayisi;

  Ilaclar(
      {this.id,
      this.ilacAdi,
      this.birimi,
      this.ikinciBirimi,
      this.katsayi,
      this.kayitSayisi});

  Ilaclar.fromJson(Map<String, dynamic> json) {
    id = json['id'];
    ilacAdi = json['ilacAdi'];
    birimi = json['birimi'];
    ikinciBirimi = json['ikinciBirimi'];
    katsayi = json['katsayi'];
    kayitSayisi = json['kayitSayisi'];
  }

  Map<String, dynamic> toJson() {
    final Map<String, dynamic> data = <String, dynamic>{};
    data['id'] = this.id;
    data['ilacAdi'] = this.ilacAdi;
    data['birimi'] = this.birimi;
    data['ikinciBirimi'] = this.ikinciBirimi;
    data['katsayi'] = this.katsayi;
    data['kayitSayisi'] = this.kayitSayisi;
    return data;
  }

  Ilaclar getObj(json) => Ilaclar.fromJson(json);
  static Future<List> fromAPI(String token,String data) async {
    final Response response =
        await post(Uri.parse(API_URL + "api/lst/ilaclarlst"),
            headers: <String, String>{
              "Authorization": "Bearer $token",
              'Content-Type': 'application/json; charset=UTF-8',
            },
            body: jsonEncode(<String, String>{"Data": data}));

    if (response.statusCode == 200) {
      return response.body.toList(new Ilaclar());
    } else if (response.statusCode == 400) {
      throw new BadRequestError(msg: response.body);
    } else {
      throw new InternalServerError(
          msg: "Servis kaynaklı bir hata oluştu.Daha sonra tekrar deneyiniz");
    }
  }
   static Future<List> fromLocal() async {
    return await (new IlaclarDb().getAll());
  }
}
